...
const Pagination = ({totalPage, page}: {totalPage: number, page:number }) => {
  ...
  return (
    <div className=' flex justify-center my-5'>
      <div className='flex gap-0.5'>
        <Link href={`/mypage/myPlanList?page=1`}
        className={cn(
          buttonVariants({
            variant: "ghost",
          }))}>
        ...
        <Link href={`/mypage/myPlanList?page=${page > 1 ? page-1 : page}`}
        className={cn(
          buttonVariants({
            variant: "ghost",
          }))}>
        ...
        {buttons}
        <Link href={`/mypage/myPlanList?page=${page < totalPage ? page+1 : page}`}
        className={cn(
          buttonVariants({
            variant: "ghost",
          }))}>
        ...
        <Link href={`/mypage/myPlanList?page=${totalPage}`}
        className={cn(
          buttonVariants({
            variant: "ghost",
          }))}>
